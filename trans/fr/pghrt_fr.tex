...
Ce document est organisé comme une série de questions/résponses, de telle sorte que chaque question et chaque section forment une suite logique. Je t'encourage fortement à le lire séquentiellement, ça se lit comme une conversation qui devrait répondre à la totalité de tes questions (incluant celles que tu ne savais pas que tu te posais), même si c'est très long. Prends ton temps et lis le à ton rythme. 
...
Non. Et c'est suffisamment important pour ne pas être relègué dans la Section \ref{MM} "Mythes et autres". \textbf{On ne peut pas comparer les dosages d'œstrogènes sans prendre en compte la voie d'administration choisie}. 1mg en pilules n'est pas la même chose qu'1mg en injections par exemple. Les différentes voies d'administration ont des propriétés différentes, ce qui affecte la quantité d'œstrogènes absorbée dans le corps (\textit{"biodisponibilité"}), à quelle vitesse, et donc la demi-vie corresponante.
...
\textbf{Le fait d'avoir des niveaux uniformes.} L'uniformité, c'est le truc important pour un THS. L'uniformité, ça veut dire que ton régime est stable, et la stabilité, c'est bien. Même les pires formes d'injections (on détaillera ça plus tard) te créeront un cycle hormonal plus uniforme que toutes les autres voies d'administration, ce qui a beaucoup d'avantages.
...
\textbf{A noter pour les seringues à insuline : ce à quoi va ressembler la mesure de ce volume peut dépendre de l'échelle utilisée pour la seringue, comme on le verra plus tard dans la Question \ref{5-18}.}
...
\subsection{Quels sont les sites d'injection sûrs ?}\label{5-5}
...
Une chose à garder en tête si tu choisis de faire l'injection dans l'abdomen, il est crucial de ne pas injecter trop profondément, ça serait plus douloureux.
...
\subsection{Est-ce que l'angle d'injection / ma méthode préférée d'injection à une incidence ?}\label{5-9}
...
La seule considération intéressante en ce qui concerne l'angle d'injection est si tu choisis de faire tes injections dans l'abdomen, vu que tu peux aller trop profondément en injectant directement à 90° de la peau si ton aiguille est trop longue (on en discute plus en détail ensuite). Le principe reste le même cependant.
...
\subsection{Quelle longueur d'aiguille dois-je utiliser pour l'injection ?}\label{5-17}
...
\subsection{Est-ce que la taille de la seringue compte ?}\label{5-18}
...
\textbf{A noter pour les seringues à insuline : il existe deux échelles de mesure différentes, ce qui peut causer un peu de confusion dans le dosage. Les seringues U-100 et les seringues U-40 calibrent leurs graduations de manière différentes à volume égal.} "U-100" et "U-40" réfèrent au nombre de graduations (ou unités) par mililitre. Les "unités" sont la manière historique de doser de l'insuline et ce sont les deux standards de calibration. Les seringues U-100, qui ont d'habitude \textbf{un capuchon orange} ,ont 100 unités par mililitre. Chaque unité a un volume de 0.01 mL, ce qui veut dire que la graduation est \textbf{équivalente} au reste des seringues. Les seringues U-40, qui ont d'habitude un \textbf{capuchon rouge}, ont 40 unités par mililitre. Chaque unité a un volume de 0.025 mL, ce qui veut dire que les graduations \textbf{ne sont pas équivalentes} aux reste des seringues. Les graduations sont généralement marquées avec le nombre d'unités et sa correspondance en mililitres, donc par exemple un volume de 0.30 mL correspondrait à 30 unités sur une seringue U-100 et 12 unités sur une seringue U-40. \textbf{Je recommande d'utiliser une seringue U-100, parce que c'est plus pratique, mais assure-toi de savoir quelle est le type de seringues que tu as !} 
...
NdT: en France, les deux substances sont considérées comme des médicaments (la testostérone en liste I et l'oestrogène en liste 2), et sont donc interdites à l'utilisation sans ordonnance. Cependant, il n'y a jamais eu à ce jour de condamnation à ce propos, donc pas la peine de trop t'inquiéter pour ça.
% TODO: localisation
...
\subsection{Est-ce que je prends de la progestérone ou une progestine ?}
...
On ne sait pas trop. Certaines personnes pensent que commencer trop tôt peut heurter le développement mammaire à long terme, mais c'est une croyance purement théorique et anecdotalement, on observe l'inverse, ce qui rend difficile la réponse à cette question. L'estimation la moins risquée est d'attendre un an de THS (soit le stage de Tanner 3 ou 4 \href{https://en.wikipedia.org/wiki/File:Tanner_scale-female.svg}{L'étape 3 ou 4 de l'échelle de Tanner}) dans l'éventualité où la progestérone ait un impact. 
...
Et je devrais le mettre dans une question à part, mais je ne veux pas casser le formatage : en lien avec les mauvaises pratiques médicales, \textbf{il N'Y a PAS de situation dans laquelle il est raisonnable pour un.e médecin de te demander de voir ou de toucher ta poitrine pour "surveiller leur croissance" ou pour quelque raison que ce soit.} C'est beaucoup moins commun dernièrement, heureusement, mais \textbf{c'est une agression sexuelle et c'est totalment inacceptable.}
...
\subsection{Quelles sont les trucs liés aux variations intersexes que je dois garder en tête ?}

Au cours de ce guide, j'ai mentionné vaguement les variations intersexes. Au-dessous figure une courte liste qui peut être utile à savoir pour toi ou pour tes proches.

\subsection{C'est quoi le truc avec le Syndrome de Klinefelter ?}

C'est une variation liée à l'intersexualité plutôt commune (au regard des mutations chromosomiques) que certaines femmes trans pourraient ne pas savoir qu'elles ont vu que ces deux ont des recouvrements. Généralement, ça se voit à un niveau de testostérone faible au début de la puberté. C'est bon de savoir le nom, juste au cas où.

\subsection{C'est quoi le truc avec le Syndrome des canaux de Müller persistants (PMDS)?}

Une nouvelle entrée pour la catégorie "Je mets ça ici parce que c'est probablement la première fois que tu entends le terme". C'est une variation liée à l'intersexualité qui peut affecter certaines femmes trans, même si on ne sait pas à quelle fréquence étant donné qu'on n'a pas de chiffres à ce propos. La présence possible d'un utérus sous-développé peut mener à des complications et des bizarreries. Tu veux probablement prendre de la progestérone pour éviter des risques de cancer de l'utérus.

\subsection{C'est quoi le truc avec le Trouble Ovotesticulaire ?}

Cette variation intersexe en particulier peut causer des fluctuation au départ du THS, ce qui peut mener à des résultats de prise de sang confus dû à la présence de tissus ovariens et testiculaires, soit séparés ou combinés en \textit{ovotestis}. Cela se présente sous différentes formes avec lequelles le THS peut interagir, au moment où tu commence à diminuer les niveaux \textit{d'hormone lutéinisante} (LH). Un utérus peut être présent ou non, plusieurs ensembles de gonades peuvent être présents, même si la variation peut ne pas être visible extérieurement.

\subsection{Quelle est la différence entre des crampes intestinales et des crampes utérines ?}\label{11-35}

Elles sont communément attribuées à tord au début de la transition comme étant un symptôme de variation intersexe. Les crampes intestinales sont étendues et se diffusent partout sur ton abdomen, alors que les crampes utérines sont très concentrées en un seul endroit juste au dessous du nombril et sont des contractions qui font comme des coups de poignard et qui arrivent en successions rapides. Comme si l'intérieur de ton corps était utilisé comme une boule à stress. Rien à voir !

\subsection{Et à propos des autres variations intersexes ?}

J'ai parlé de quelques unes des plus notables, mais il y a bien plus d'expressions et de manières de les tester, qui dépassent de loin l'objectif de ce guide. Anecdotalement, la prévalence de ces variations est plus élevée parmi les personnes trans, donc une familiarité de base avec ça est toujours utile.
...
Non. Les bodybuilders et les athlètes l'aiment parce qu'avoir plus d'énergie permet de faire plus de choses avant d'être fatigué.e. Ce ne sont pas les seul.es à l'utiliser parce qu'en fait c'est le 1er supplément vraiment efficace en terme d'effet et qui sont vraiment soutenu par la recherche.
...
\begin{enumerate}
  ...
  \item \href{https://web.archive.org/web/20260310142419/https://hrtcafe.net/}{HRT Cafe} - Aggrégateur de Ressources THS
...
\noindent 2026-02-18: Correction de quelques fautes de la publication initiale, ajout de détails sur les injections abdominales sur les questions \ref{5-5}, \ref{5-9} et \ref{5-17}. 25.2k mots.
